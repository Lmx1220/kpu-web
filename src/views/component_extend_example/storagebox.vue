...
<template>
  <div>
    <PageHeader title="储存箱" content="StorageBox" />
    <PageMain title="你可以将页面中的数据（object/array）储存到该组件内，并在需要的时候取出来">
      <ElAlert show-icon title="该组件需要搭配业务场景使用。" :closable="false" />
      <StorageBox :data="data" name="box1" mt2.5 />
    </PageMain>
    <PageMain title="推荐场景：具有复杂筛选项的页面，并记录常用的筛选条件">
        ...
        <ElForm :model="search" size="default" label-width="120px">
          ...
          <ElFormItem>
            <ElSpace>
              ...
              <StorageBox :data="search" name="box2" title="我的快捷筛选" @take-out="takeOut">
                <ElButton link>
                  保存当前筛选
                </ElButton>
              </StorageBox>
