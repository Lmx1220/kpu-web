...
const files = ref([
  {
    name: 'kpu-admin.png',
    url: 'https://lmx1220.github.io/kpu-web/assets/logo-BGVN89r2.png',
  },
])
...
async function upload(option: any) {
  const response = await requestClient.upload(option.action, {
    file: option.file,
    ...option.data,
  })
}
...
<template>
  <div>
    ...
    <KpuPageMain title="单图上传">
      <ImageUpload v-model="image" action="anyone/file/upload" :data="{ bizType: 'DEF__APPLICATION__LOGO' }" :http-request="upload" name="image" :width="250" :height="150" @on-success="handleSuccess1" />
    </KpuPageMain>
