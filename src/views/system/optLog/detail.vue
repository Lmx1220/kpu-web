...
function goBack() {
  if (settingsStore.settings.tabbar.enable && settingsStore.settings.tabbar.mergeTabsBy !== 'activeMenu') {
    tabbar.close({ name: 'SystemOptLogList' })
  }
  else {
    router.push({ name: 'SystemOptLogList' })
  }
}
...
<template>
  <div>
    <page-header :title="route.name === 'routerName' ? '新增登录日志' : '编辑登录日志'">
      ...
      <el-row>
        <el-col :lg="16" :md="24">
          <DetailForm
            ...
            :type="(route.query?.type as 'add'|'edit'| 'view'| undefined)"
          />
        </el-col>
      </el-row>
    </page-main>
