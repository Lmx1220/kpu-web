...
import type { DefGenTablePageQuery, DefGenTableResultVO } from '@/api/modules/devOperation/developer/model/defGenTableModel'
import crudGenTable, { downloadZip, generatorCode, syncField } from '@/api/modules/devOperation/developer/defGenTable.ts'
...
const {
  ...
  onSortChange,
  resetQuery,
} = usePagination<DefGenTablePageQuery>()
...
const data: Ref<DataConfig<DefGenTablePageQuery, DefGenTableResultVO>> = ref({
  loading: false,
...
function onEdit(row?: any) {
  ...
  if (data.value.formMode === 'router') {
    router.push({
      name: 'DevOperationDeveloperDefGenTableEdit',
      params: {
        id: ids.join(',') || '',
      },
      query: {
