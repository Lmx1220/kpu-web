...
import crudGenTableColumn, { syncField } from '@/api/modules/devOperation/developer/defGenTableColumn.ts'
import type { DefGenTableColumnPageQuery } from '@/api/modules/devOperation/developer/model/defGenTableColumnModel.ts'
...
async function getDataList(current?: number) {
  ...
  const params = getParams<DefGenTableColumnPageQuery>({
    ...data.value.search,
  })
...
function onSync(row?: any) {
  ElMessageBox.confirm('确认同步吗？', '确认信息').then(() => {
    syncField(props.id as string, row.id).then(() => {
      getDataList()
      ElMessage.success({
        message: '同步成功',
        center: true,
