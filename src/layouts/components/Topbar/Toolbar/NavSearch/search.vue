...
<template>
  <KModal ref="searchResultRef" v-model="isShow" border :footer="settingsStore.mode === 'pc'" :closable="false" class="w-full lg-max-w-2xl" content-class="flex flex-col p-0 min-h-auto" header-class="p-0" footer-class="p-0">
    <template #header>
      <div class="h-12 flex flex-shrink-0 items-center">
        ...
        <div v-if="settingsStore.mode === 'mobile'" class="h-full w-14 flex-center border-s">
          <KIcon name="i-carbon:close" :size="18" @click="isShow = false" />
        </div>
      </div>
    </template>
    <template #footer>
