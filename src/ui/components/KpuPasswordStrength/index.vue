...
<template>
  <div class="w-full flex-center gap-2">
    ...
    <KpuTooltip>
      <KpuIcon name="i-ri:question-line" class="cursor-help text-sm text-muted-foreground" />
      <template #content>
        <div class="py-1">
          ...
          <ul class="text-sm text-muted-foreground space-y-1">
            <li class="flex-center-start gap-1" :class="{ 'text-green-600': hasLength }">
              <KpuIcon :name="hasLength ? 'i-carbon:checkmark' : 'i-carbon:close'" />
              长度至少为8个字符
            </li>
            <li class="flex-center-start gap-1" :class="{ 'text-green-600': hasUppercase }">
              <KpuIcon :name="hasUppercase ? 'i-carbon:checkmark' : 'i-carbon:close'" />
              包含大写字母
            </li>
            <li class="flex-center-start gap-1" :class="{ 'text-green-600': hasLowercase }">
              <KpuIcon :name="hasLowercase ? 'i-carbon:checkmark' : 'i-carbon:close'" />
              包含小写字母
            </li>
            <li class="flex-center-start gap-1" :class="{ 'text-green-600': hasNumber }">
              <KpuIcon :name="hasNumber ? 'i-carbon:checkmark' : 'i-carbon:close'" />
              包含数字
            </li>
            <li class="flex-center-start gap-1" :class="{ 'text-green-600': hasSpecialChar }">
              <KpuIcon :name="hasSpecialChar ? 'i-carbon:checkmark' : 'i-carbon:close'" />
              包含特殊字符
            </li>
          </ul>
        </div>
